...
# What every tool task says about *how* to answer. `process` is a real reward now, so the
# prompt has to state the rule it scores. The first gate run under the split scored a
# perfect 1.0 on `outcome` for all thirteen tasks and lost `process` on six of them, while
# every prompt said only "use the Tastytrade MCP tools" and never said that anything else
# was out of bounds. The exception is `place-limit-order`, the one task that already spelt
# the rule out. This is that task's sentence, generalised to the rest.
#
