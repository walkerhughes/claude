...
# agent against the mock FRED API, and fails unless the rewards clear a
# threshold. This is the merge gate on the tool surface.
#
...
# model involved. That catches a broken verifier. Only this catches a server a
# real agent cannot drive.
#
# Hub results are named `ci-evals-fred`. That is the repo-wide convention,
...
echo "==> Building fred-bench from the working tree"
( cd "$ROOT" && docker build -q -f evals/environment/Dockerfile -t fred-bench . )
...
if ! python3 "$ROOT/evals/check_reward.py" "$result" "$JOB_NAME" --min-mean "$MIN_MEAN"; then
    ...
    # `process`. A bare `cat` of the verifier output prints ten anonymous
